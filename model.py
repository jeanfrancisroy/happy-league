...
class Field:
    ...
    def key(self): return (self.stadium, self.name, self.time, self.date.key() )    
...
class League:
    ...
    def __init__(self, divisionL ): #, fieldL, restrictionL ):
        self.name = ""
...
#class Ligue(League):pass


def roundRobin(nTeam,nMatch=None):
    if nMatch is None: nMatch = nTeam-1
#    print nTeam
    if nTeam % 2 == 0:
...
#        nMatch_ = nMatch + 1
    ...
    i = 0
    while True:
#        if countMatch( nTeam, nMatch, matchL ): break
...
#        print idxL_
        for j in range(len(idxL_)/2):
            ...
            if match[0] >=0 and match[1] >= 0:
                matchL.append( match )
            if countMatch( nTeam, nMatch, matchL ): return matchL
        i += 1
    return matchL
...
class DivisionHalf(Division):
    ...
    def __init__(self, name, teamL1, teamL2):
        self.name = name
#        for team in teamL1 : print team
#        for team in teamL2 : print team
...
class Restriction:
    ...
    def isActive( self, field ):
        ok = True
        try: ok &= field.stadium in self.stadiumS
        except  TypeError : pass
        try: ok &= field.name in self.fNameS
        except  TypeError : pass
    ...
    def __str__(self):
        ...
        if self.timeMin is not None or self.timeMax is not None:
            s = ''
            if self.timeMin is not None: s += str(self.timeMin)
            s += '...'
            if self.timeMax is not None: s += str(self.timeMax)
            restrL.append(s)
...
class TeamGrpConflict:
    ...
    def _getPenRatio(self):
        ...
        if not self.sameStadium:
            ...
            elif dt > 60.*5. : pen = 0.3 # playable but over 5h delay
            else : pen = 0.3 * dt/(60.*5.) # penalty is linearly proportional to delay
    
        return pen
    
class Config:
    """
    There are 4 kind of restrictions that are taken into account in the schedule generator.
    ...
    * Uniformity penalties : penalties incurred when some teams have too many games in a generally inconvenient field.
    ...
    def getGrpList(self):
        ...
        grpL = np.array( grpL, dtype=np.object )  # transform into numpy array so we can batch-index elements.
        return grpL
    ...
    def getGrpPen(self):
        ...
        for i in range(n):
            ...
            for j in range(i):
                ...
                if grpConflict is None : continue
                else : 
    ...
    def getGrpConflictMat(self):
        ...
        """
        n = len(self.fieldL)
        conflictMat = np.zeros( (n,n), dtype=np.object )
        conflictMat[:]  = None
        ...
        for i in range(n):
            for j in range(i):
                ...
                if fieldA.date != fieldB.date : continue # not the same day => no conflict
    ...
    def getUnifStruct(self):
        ...
        pastUnifCount = np.zeros( (len( idMap ), len(self.teamL)+1), dtype=np.int )
        for team in self.teamL:
...
#            pass
        ...
        scheduleMat = np.array( matchL,dtype=np.int )
        return scheduleMat
...
class ObjSrv:
    ...
    def __getitem__(self, obj ):
        try: return self.objD[obj.key()] # try to return an object of the same key
        except KeyError:
            if self.lock :
